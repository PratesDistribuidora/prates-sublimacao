...
Visual limpo, profissional e discreto.
...
[data-testid="stWarning"] {
    ...
    border: 1px solid #3a2a0a !important;
    border-radius: 6px !important;
    font-size: 13px !important;
}
[data-testid="stError"] {
...
with st.sidebar:
    ...
    st.markdown('<hr style="border-color:#252932;margin:8px 0 10px">', unsafe_allow_html=True)
    ...
    st.markdown('<p style="text-align:center;color:#3a4050;font-size:11px;margin:0">Prates Sublimação · Macaé/RJ · v6.0</p>', unsafe_allow_html=True)
...
if pagina == "📊 Dashboard":
    ...
    if not kpis:
        ...
        with col1:
            ...
            fig.update_layout(barmode='group', **CHART,
                legend=dict(orientation='h', y=1.1, font=dict(size=10, color='#6b7280')))
        ...
        with col2:
            ...
            fig2 = px.bar(dt, x='Tecido', y='Margem', color='Tecido',
                          color_discrete_sequence=['#2d7a4f','#2563eb','#d97706','#7c3aed'])
        ...
        with col3:
            ...
            fig3 = px.pie(dp, names='Tecido', values='SKUs', hole=0.5,
                          color_discrete_sequence=['#2d7a4f','#2563eb','#d97706','#7c3aed'])
        ...
        with col4:
            ...
            st.dataframe(
                df.nlargest(10,'Custo Final')[['Modelo','Tecido','Cor','Tamanho','Custo Final']].reset_index(drop=True),
        ...
        st.dataframe(pd.DataFrame([{
            'Modelo': f['modelo'],
            'Faccionista Ativa': f['faccionista_ativa'],
    ...
    with t1:
        ...
        if c1_ and tm1_:
            ...
            if calc:
                ...
                dc = {
                    'Faixa':     ['Super Revenda +20%','Atacado +35%','Varejo +50%','Preço do cliente'],
                    'Preço':     [calc['super_revenda'], calc['atacado'], calc['varejo'], preco_cli],
                    'Lucro':     [round(calc['super_revenda']-custo,2), round(calc['atacado']-custo,2),
                                  round(calc['varejo']-custo,2), lucro],
                ...
                if lucro < 0:
                    st.error(f"Abaixo do custo! Mínimo sem prejuízo: {fmt(custo)}")
                elif margem < 10:
                    st.warning(f"Margem muito baixa ({margem:.1f}%). Sua faixa SR padrão é 20%.")
                else:
                    st.success(f"Margem de {margem:.1f}% — dentro do esperado.")
        ...
        if c2_ and tm2_:
            ...
            if calc2:
                ...
                fig = px.line(pd.DataFrame({'Margem (%)': margens, 'Preço (R$)': precos}),
                              ...
                              color_discrete_sequence=['#2d7a4f'])
                fig.add_vline(x=mg_des, line_dash="dash", line_color="#d97706",
                              annotation_text=f"  {mg_des}%", annotation_font_color="#d97706")
...
# ══ LOTE ═══════════════════════════════════════
    ...
    if not skus:
        ...
        for idx in range(len(st.session_state.ll)):
            ...
            if c6.button("✕", key=f"lt_r{idx}"):
                st.session_state.ll.pop(idx); st.rerun()
            if tec and mod and co and ta__:
                inputs.append((tec, co, mod, ta__, kg))
        ...
        if st.button("Calcular Lote") and inputs:
            ...
            if res:
                ...
                c1,c2,c3,c4,c5,c6 = st.columns(6)
                c1.metric("Total Kg",       f"{tkg:.1f}kg")
                c2.metric("Total Peças",    tpcs)
                c3.metric("Custo Total",    fmt(tct))
                c4.metric("Faturamento SR", fmt(tft))
                c5.metric("Lucro Total",    fmt(tlc))
                c6.metric("Margem Real",    fpct(mgr))
...
# ══ TABELA CLIENTE ══════════════════════════════
    ...
    if not cat:
        ...
        fx = st.radio("Faixa de Preço para o Cliente:",
                      ["Super Revenda","Atacado","Varejo","Todas as Faixas"], horizontal=True)
        ...
        with cp:
            if st.button("Gerar PDF para Cliente"):
                ...
                for _, row in dff.iterrows():
                    if fx == "Todas as Faixas":
                        dados_pdf.append({'Modelo':row['Modelo'],'Tecido':row['Tecido'],
                ...
                st.download_button("⬇️ Baixar PDF", pdf,
                    file_name=f"tabela_cliente_{date.today()}.pdf", mime="application/pdf")
        with cw:
        ...
        with cc_:
            ...
            st.download_button("⬇️ Exportar CSV", dff_csv.to_csv(index=False).encode('utf-8-sig'),
                               file_name=f"tabela_{date.today()}.csv", mime="text/csv")
...
# ══ RELATÓRIO MENSAL ════════════════════════════
    ...
    with tp:
        ...
        if not regs:
            ...
            c2.metric("Peças",    int(df['qtd_pecas'].sum()))
            c3.metric("Receita",  fmt(rec))
            c4.metric("Custo",    fmt(cst))
            c5.metric("Lucro",    fmt(luc))
            c6.metric("Margem",   fpct(luc/rec if rec else 0))
            col1, col2 = st.columns(2)
            with col1:
            ...
            with col2:
                ...
                fig2 = px.pie(df2, names='faixa', values='receita', hole=0.5,
                              color_discrete_sequence=['#2d7a4f','#2563eb','#d97706'])
    ...
    with tr:
        ...
        if not regs:
            ...
            df = pd.DataFrame(regs)
            st.dataframe(df[['data','numero_pedido','modelo','faixa','tecido',
                             'qtd_pecas','receita','custo','lucro','observacao']],
                        use_container_width=True, hide_index=True)
...
# ══ CONFIGURAÇÕES ════════════════════════════════
    ...
    with tp:
        ...
        if st.button("Salvar Parâmetros"):
            for k,v in [('frete_pct',fr),('outros_pct',ou),('embalagem',em),
                        ('margem_sr',ms_),('margem_atacado',ma_),('margem_varejo',mv_)]:
                set_parametro(k,v)
        ...
        for fo in get_fornecedores():
            with st.expander(f"{fo['tecido']} — {fo['cor']}   |   Ativo: {fo['fornecedor_ativo']}   |   {fmt(get_preco_kg(fo['tecido'],fo['cor']))}/kg"):
                ...
                if st.button("Salvar",key=f"sf{fo['id']}"):
                    ...
                    update_fornecedor(fo['id'],{'f1_nome':f1n,'f1_preco':f1p,'f2_nome':f2n,
                        'f2_preco':f2p,'f3_nome':f3n,'f3_preco':f3p,'fornecedor_ativo':fa_})
        ...
        for f in get_faccionistas():
            with st.expander(f"{f['modelo']}   |   Ativa: {f['faccionista_ativa']}   |   {fmt(get_preco_costura(f['modelo']))}/peça"):
                ...
                if st.button("Salvar",key=f"sfac{f['id']}"):
                    ...
                    update_faccionista(f['id'],{'f1_nome':fn1,'f1_preco':fp1,'f2_nome':fn2,
                        'f2_preco':fp2,'f3_nome':fn3,'f3_preco':fp3,'faccionista_ativa':faa})
...
# ══ HISTÓRICO ════════════════════════════════════
    ...
    with tv:
        h = get_historico()
        if not h:
            st.info("Nenhum registro. O sistema registra automaticamente ao salvar em Configurações.")
        else:
            dh = pd.DataFrame(h)
...
# ══ IMPORTAR ═════════════════════════════════════
    ...
    st.markdown('<hr style="border-color:#252932;margin:12px 0">', unsafe_allow_html=True)
    c1,c2,c3,c4 = st.columns(4)
    c1.metric("SKUs",         len(get_skus()))
    c2.metric("Fornecedores", len(get_fornecedores()))
    c3.metric("Faccionistas", len(get_faccionistas()))
    c4.metric("Histórico",    len(get_historico()))
